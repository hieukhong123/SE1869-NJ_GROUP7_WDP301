...
const HotelInfoCard = ({ hotel, reviews = [] }) => {
    ...
    return (
        <div className="card bg-base-100 shadow-xl sticky top-8 overflow-hidden">
            {/* Image Gallery */}
                ...
                {photos.length > 1 && (
                    <div className="absolute bottom-4 left-4 right-4 flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                        {photos.map((photo, index) => (
                            ...
                            >
                                <img src={photo} className="w-full h-full object-cover" alt={`Hotel thumbnail ${index + 1}`} />
                            </button>
                        ))}
                    </div>
                )}
            ...
            <div className="card-body p-6">
                ...
                <div className="flex items-start justify-between gap-4">
                    <h2 className="card-title text-2xl font-black text-slate-800 leading-tight">
                        {hotel.name}
                    </h2>
                    <div className="flex flex-col items-end shrink-0">
                        <div className="bg-primary text-white font-black h-12 w-12 rounded-xl flex items-center justify-center text-lg shadow-lg shadow-primary/20">
                ...
                <div className="flex items-start gap-2 text-slate-500 mt-2">
                    ...
                    <div>
                        <p className="text-sm font-bold text-slate-700">{hotel.address}</p>
                ...
                {hotel.description && (
                    <div>
                        <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-2">About this hotel</h3>
                ...
                <div className="mt-6 space-y-3">
                    <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                        <span className="text-[10px] font-black text-slate-400 uppercase">Contact Phone</span>
                        <span className="text-sm font-bold text-slate-700">{hotel.hotelPhone}</span>
                    </div>
                    <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100">
                        <span className="text-[10px] font-black text-slate-400 uppercase">Email</span>
                        <span className="text-sm font-bold text-slate-700">{hotel.hotelEmail}</span>
                    </div>
                </div>
            </div>
        </div>
