...
const AdminLayout = () => {
  ...
  // Early return to prevent "flash" of protected content
  if (!user || !token || (user.role !== 'admin' && user.role !== 'staff')) {
    return null;
  }
      ...
      <div className="drawer-side z-40">
        ...
        <div className="flex flex-col w-72 min-h-screen bg-white border-r border-gray-200">
          ...
          <div className="px-6 py-6 border-t border-gray-100 bg-gray-50/30">
            {/* User Profile - AT BOTTOM AS REQUESTED */}
            <div className="flex items-center gap-3 mb-6 px-2">
              <div className="w-10 h-10 rounded-sm bg-white flex items-center justify-center text-orange-800 font-medium overflow-hidden border border-gray-200 shadow-sm">
                {user?.avatar ? (
                  <img src={user.avatar} alt="avatar" className="w-full h-full object-cover" />
                ) : (
                  <span className="text-sm uppercase font-semibold">
                    {user?.fullName?.split(' ').map(n => n[0]).join('') || <User size={18} />}
                  </span>
                )}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 min-w-0">
                  <p className="text-[9px] uppercase tracking-wider text-orange-800 font-medium truncate">
            ...
            <button
              ...
              className="flex items-center gap-4 w-full px-4 py-3 text-xs tracking-widest uppercase font-medium text-red-500 hover:bg-red-50 hover:text-red-700 transition-colors border border-dashed border-red-200 rounded-sm"
            >
