...
import { PencilSimple, Trash, Plus, CircleNotch, Users } from '@phosphor-icons/react';
...
const UserList = () => {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to remove this user from the directory? This action cannot be undone.')) {
            try {
    ...
    const columns = [
        { 
            ...
            cell: (info) => (
                <div>
                    <span className="font-medium text-gray-900 block">{info.getValue() || 'Not Provided'}</span>
                    <span className="text-[10px] text-gray-400 uppercase tracking-widest">{info.row.original.userName}</span>
                </div>
            )
        },
        { 
            ...
            cell: (info) => <span className="text-gray-600 font-light">{info.getValue()}</span>
        },
        {
            accessorKey: 'role',
            header: 'Role',
            cell: ({ row }) => {
                ...
                return (
                    <span className={`text-[10px] uppercase tracking-widest font-medium px-2 py-1 rounded-sm border ${
                        role === 'admin' ? 'bg-orange-50 border-orange-200 text-orange-800' : 'bg-gray-50 border-gray-200 text-gray-600'
                    }`}>
            ...
            <div className="max-w-7xl mx-auto">
                ...
                {users.length === 0 ? (
                    <div className="bg-white border border-gray-200 border-dashed rounded-sm py-32 flex flex-col items-center justify-center text-center px-4">
                        <Users size={48} weight="light" className="text-gray-300 mb-6" />
                        <h3 className="text-xl font-serif text-gray-900 mb-2">Directory is Empty</h3>
